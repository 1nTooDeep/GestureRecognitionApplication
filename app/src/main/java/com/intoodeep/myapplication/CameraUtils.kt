...
object CameraUtils {
    ...
    fun convertImageToColorfulResizedBitmap(image: Image, newWidth: Int, newHeight: Int): Bitmap {
        ...
        // 计算缩放比例
        val scaleX = srcWidth.toFloat() / newWidth.toFloat()
        val scaleY = srcHeight.toFloat() / newHeight.toFloat()

        // 将YUV_420_888格式转换为ARGB_8888格式的彩色Bitmap并进行缩放
        ...
        for (y in 0 until newHeight) {
            for (x in 0 until newWidth) {
                ...
                // 计算在原始图像中的UV索引
                val uvRowIndex = (srcY / 2) * uvStride
                val uvIndex = uvRowIndex + (srcX / 2) * uvPixelStride

                // 获取对应位置的YUV值
                val yValue = yBuffer.get(srcY * yStride + srcX).toInt() and 0xFF
                ...
                // 将RGB值限制在0-255范围内
                val red = Math.max(0, 255.coerceAtMost(r.toInt()))
                val green = Math.max(0, 255.coerceAtMost(g.toInt()))
                val blue = Math.max(0, 255.coerceAtMost(b.toInt()))

                // 将RGB值应用于生成的彩色Bitmap
                resizedBitmap.setPixel(x, y, Color.argb(255,red, green, blue))
            }
        }
